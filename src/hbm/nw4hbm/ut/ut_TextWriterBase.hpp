...
namespace nw4hbm { namespace ut
{
	...
	public:
		...
		static int VSNPrintf(charT *buffer, u32 count,
		                            const charT *format, std::va_list arg)
		{
			return sizeof(charT) == sizeof(char)
			         ? std::vsnprintf((char *)buffer, count,
			                          (const char *)format, arg)
			         : std::vswprintf((wchar_t *)buffer, count,
		...
		f32 AdjustCursor(f32 *xOrigin, f32 *yOrigin, const charT *str,
		                 int length);
