...
#include "../math/math_arithmetic.hpp" // math::FAbs
#include "../math/math_triangular.hpp"
...
u32 CalcOffsetTevStageAry(const detail::BitGXNums &bitGXNums)
{
	return CalcOffsetIndTexSRTAry(bitGXNums)
	     + (sizeof(TevStage) + 4) * bitGXNums.indSRT; // hm?
}
...
Material::Material(const res::Material *pRes, const ResBlockSet &resBlockSet)
{
	...
	const TexCoordGen *resTexCoordGens =
	...
	if (mpGXMem)
	{
		...
		if (texMapNum)
		{
			const res::Texture *textures =
				detail::ConvertOffsToPtr<res::Texture>(resBlockSet.pTextureList,
			                                           12);
                            ...
                            bool allocAlpComp, bool allocBlendMode)
{
	...
	mpGXMem = Layout::AllocMemory(
		4ul
			* (matColNum + chanCtrlNum + texCoordGenNum + tevSwapNum
	           + alpCompNum + blendModeNum + indStageNum)
		+ 0x14ul * texSRTNum + 0x20ul * texMapNum + 0x14ul * indSRTNum
		+ 0x10ul * tevStageNum);
...
{
	std::strncpy(mName, name, ARRAY_LENGTH(mName));
}
...
bool Material::SetupGX(bool bModVtxCol, u8 alpha)
{
	...
	if (bSetTexMtx)
	{
		...
		for (u8 i = 0; i < mGXMemNum.texSRT; i++)
		{
			if (bUseTexMtx[i])
			{
				...
				GXLoadTexMtxImm(texMtx, GetTexMtx(i), GX_TG_MTX2x4);
			}
		}
	}
	...
	else
	{
		...
		for (u8 id = 0; id < tevStageNum; id++)
		{
			...
			GXSetTevColorOp(tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_TEV_SCALE_0, 1,
			                GX_TEVPREV);
			GXSetTevAlphaOp(tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_TEV_SCALE_0, 1,
			                GX_TEVPREV);
