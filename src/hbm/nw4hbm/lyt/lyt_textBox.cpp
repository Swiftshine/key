...
#include "lyt_types.hpp" // detail::ConvertOffsToPointer
#include "lyt_resourceAccessor.hpp"

#include "../math/math_types.hpp" // math::VEC2
#include "../ut/ut_CharStrmReader.hpp"
#include "../ut/ut_CharWriter.hpp"
#include "../ut/ut_Color.hpp"
#include "../ut/ut_Font.hpp"
#include "../ut/ut_inlines.hpp"
#include "../ut/ut_ResFont.hpp"
#include "../ut/ut_Rect.hpp"
#include "../ut/ut_RuntimeTypeInfo.hpp"
#include "../ut/ut_TagProcessorBase.hpp"
#include "../ut/ut_TextWriterBase.hpp"
...
namespace
{
	...
	int CalcLineStrNum(f32 *pWidth, ut::TextWriterBase<charT> *pTextWriter,
	                   const charT *str, int length, f32 maxWidth,
	                   bool *pbOver);
...
u8 ClampColor(s16 colVal)
{
	return CLAMP(colVal, 0, 0xff);
}

template <typename charT>
int CalcLineStrNum(f32 *pWidth, ut::TextWriterBase<charT> *pTextWriter,
                   const charT *str, int length, f32 maxWidth, bool *pbOver)
{
	ut::Rect rect;
                     ...
                     const charT *str, int length, f32 maxWidth, bool *pbOver)
{
	...
	*pbOver = false;
	...
	for (char16_t code = reader.Next();
	     ...
	     prStrPos = static_cast<const charT *>(reader.GetCurrentPos()),
                        ...
                        const charT *str, int length, f32 maxWidth)
{
	...
	do
	{
		ut::Rect rect;
		bool bOver;
		int read = CalcLineRectImpl(&rect, pTextWriter, pos, remain, maxWidth,
		                              &bOver);
	...
	Pane(pBlock)
{
	...
	const char *fontName = detail::ConvertOffsToPtr<char>(
	...
	if (void *pMemMaterial = Layout::AllocMemory(sizeof(Material)))
	{
		const u32 *matOffsTbl =
			detail::ConvertOffsToPtr<u32>(resBlockSet.pMaterialList, 12);
