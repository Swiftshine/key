...
namespace
{
	...
	void SetTagProcessorImpl(Pane *pPane,
	                         ut::TagProcessorBase<wchar_t> *pTagProcesssor);
...
bool Layout::Build(const void *lytResBuf, ResourceAccessor *pResAcsr)
{
	...
	void *dataPtr =
		CONVERT_OFFSET_TO_PTR(void, lytResBuf, fileHead->headerSize);

	for (int i = 0; i < fileHead->dataBlocks; i++)
	{
		res::DataBlockHeader *pDataBlockHead =
			static_cast<res::DataBlockHeader *>(dataPtr);
		...
		case 'mat1':
			resBlockSet.pMaterialList =
				static_cast<res::MaterialList *>(dataPtr);
			break;
		...
		case 'grp1':
			...
			else if (mpGroupContainer && groupNestLevel == 1)
			{
				if (Group *pGroup = CreateObject<Group>(
						reinterpret_cast<const res::Group *>(pDataBlockHead),
						mpRootPane))
				{
	...
	for (int i = 0; i < pFileHead->dataBlocks; i++)
	{
		...
		case 'pai1': // painting? idk
			...
			case res::AnimationInfo::ANIM_INFO_MATERIAL_TEXTURE_PATTERN:
				...
				{
					pInfoBlock = reinterpret_cast<const res::AnimationBlock *>(
						pDataBlockHead);
                           ...
                           const ResBlockSet &resBlockSet)
{
	...
	{
		const res::Picture *pResPic =
			static_cast<const res::Picture *>(dataPtr);
	...
	{
		const res::Bounding *pResBounding =
			static_cast<const res::Bounding *>(dataPtr);
