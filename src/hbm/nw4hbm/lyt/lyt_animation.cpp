...
namespace
{
	...
	u16 GetStepCurveValue(f32 frame, const res::StepKey *keyArray,
	                      u32 keySize);
	f32 GetHermiteCurveValue(f32 frame, const res::HermiteKey *keyArray,
	                         u32 keySize);
	...
	void AnimateMaterialColor(Material *pMaterial,
	                          ...
	                          const u32 *animTargetOffsets, f32 frame);
	void AnimateTextureSRT(Material *pMaterial,
	                       const res::AnimationInfo *pAnimInfo,
	                       const u32 *animTargetOffsets, f32 frame);
	void AnimateTexturePattern(Material *pMaterial,
	                           ...
	                           const u32 *animTargetOffsets, f32 frame,
	                           void **tpls);
	void AnimateIndTexSRT(Material *pMaterial,
	                      const res::AnimationInfo *pAnimInfo,
	                      const u32 *animTargetOffsets, f32 frame);
}
                    ...
                    const u32 *animTargetOffsets, f32 frame)
{
	for (int i = 0; i < pAnimInfo->num; i++)
	{
		...
		const res::HermiteKey *keys = detail::ConvertOffsToPtr<res::HermiteKey>(
...
AnimationLink *detail::FindAnimationLink(AnimationLink::LinkList *pAnimList,
