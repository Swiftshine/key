...
#include "mix.h"
#include "syn.h"
#include "synenv.h" // __HBMSYNSetupVolumeEnvelope
#include "synmix.h"
#include "synpitch.h"
#include "synsample.h" // __HBMSYNSetupSample
#include "synvoice.h"
...
static void __HBMSYNProgramChange(HBMSYNSYNTH *synth, u8 midiChannel,
                                  u8 program)
{
	OSAssert_Line(54, synth);
	OSAssert_Line(55, midiChannel < 16);
	OSAssert_Line(56, program < 128);
...
void __HBMSYNSetController(HBMSYNSYNTH *synth, u8 midiChannel, u8 function,
                           u8 value)
{
	OSAssert_Line(67, synth);
	OSAssert_Line(68, midiChannel < 16);
	OSAssert_Line(69, function < 128);
	OSAssert_Line(70, value < 128);
...
static void __HBMSYNResetController(HBMSYNSYNTH *synth, u8 midiChannel)
{
	OSAssert_Line(102, synth);
	OSAssert_Line(103, midiChannel < 16);
...
static void __HBMSYNNoteOff(HBMSYNSYNTH *synth, u8 midiChannel, u8 keyNum)
{
	...
	OSAssert_Line(138, synth);
	OSAssert_Line(139, midiChannel < 16);
	OSAssert_Line(140, keyNum < 128);
...
static void __HBMSYNNoteOn(HBMSYNSYNTH *synth, u8 midiChannel, u8 keyNum,
                           u8 keyVel)
{
	OSAssert_Line(162, synth);
	OSAssert_Line(163, midiChannel < 16);
	OSAssert_Line(164, keyNum < 128);
	OSAssert_Line(165, keyVel < 128);
...
static void __HBMSYNMidiIn(HBMSYNSYNTH *synth, u8 *input)
{
	...
	OSAssert_Line(275, synth);
	OSAssert_Line(276, input);
