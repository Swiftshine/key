#ifndef FLUFF_KDTREEUTIL_H
#define FLUFF_KDTREEUTIL_H


#include "types.h"
#include <utility>

#include <nw4r/math.h>

typedef std::pair<bool, f32> SplitInfo;
typedef std::pair<nw4r::math::VEC2, nw4r::math::VEC2> PointPair;

/// @note Size: `0x8`
struct KdTreeSplitInfo {
    /* 0x0 */ bool mSplitY;
    /* 0x4 */ f32 mMidpoint;
};

struct KdTreeBounds {
    inline KdTreeBounds() {
        mMinX = 0.0f;
        mMinY = 0.0f;
        mMaxX = 0.0f;
        mMaxY = 0.0f;
    }

    /* 0x0 */ f32 mMinX;
    /* 0x4 */ f32 mMinY;
    /* 0x8 */ f32 mMaxX;
    /* 0xC */ f32 mMaxY;
};

namespace KdTreeUtil {
    enum NodePlacement {
        eNodePlacement_Child1  = 0,
        eNodePlacement_Child2  = 1,
        eNodePlacement_Self    = 2
    };

    /**
     * @note Address: 0x80048354
     * @note Size: 0x64
     */
    s32 DetermineNodePlacement(KdTreeSplitInfo& rSplitInfo, nw4r::math::VEC2& rPoint);

    /**
     * @note Address: 0x800483b8
     * @note Size: 0x64
     */

    s32 DetermineNodePlacementStrictly(KdTreeSplitInfo& rSplitInfo, PointPair& rPoints);

    /**
     * @note Address: 0x80048464
     * @note Size: 0x84
     */
    s32 DetermineNodePlacementByProximity(KdTreeSplitInfo& rSplitInfo, PointPair& rPoints);

    /**
     * @note Address: 0x800484e8
     * @note Size: 0x74
     */
    s32 DetermineNodePlacement(KdTreeSplitInfo& rSplitInfo, PointPair& rPoints);
}


#endif
