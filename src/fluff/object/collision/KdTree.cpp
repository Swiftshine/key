...
void KdTreeNode::AddColDataSeg(ColDataSeg* pColDataSeg) {
    KdTreeNode* node = pColDataSeg->mTreeNode;

    if (node == nullptr) {
        return;
    }

    node->Add(pColDataSeg);
}

void KdTreeNode::RemoveColDataSeg(ColDataSeg* pColDataSeg) {
    bool b;
    KdTreeNode* node = pColDataSeg->mTreeNode;
    ...
    if (nodeData == nullptr) {
        b = false;
    } else if (nodeData == pColDataSeg) {
        node->SetColDataSeg(static_cast<ColDataSeg*>(pColDataSeg->GetNext()));
        b = true;
        ...
        for (
            ...
            if (cur == pColDataSeg) {
                nodeData->SetNext(pColDataSeg->GetNext());
                b = true;
                pColDataSeg->SetNext(nullptr);
                node->SetColDataSegCount(node->GetColDataSegCount() - 1);
                goto end;
...
ColDataSeg* KdTree::HitResult::GetCurrentColDataSeg() {
    ...
    if (mColDataSegCount >= numNodes) {
        return nullptr;
    }

    mCurrentColDataSeg = mNodes[0]->GetColDataSeg();

    while (mCurrentColDataSeg == nullptr) {
        mColDataSegCount++;
        
        if ((uint)mColDataSegCount >= numNodes) {
            mColDataSegCount--;
            result = nullptr;
            goto exit;
        } else {
            mCurrentColDataSeg = mNodes[mColDataSegCount]->GetColDataSeg();
        }
    }

    result = mCurrentColDataSeg;
    mCurrentColDataSeg = static_cast<ColDataSeg*>(result->mNext);
exit:
    return result;
}
