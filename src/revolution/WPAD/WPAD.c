...
#include <revolution/WPAD/lint.h>
#include <revolution/WPAD/WPADHIDParser.h>
#include <revolution/WPADMem.h>
...
#define CURRENT_AFH_CHANNEL_PHYS_ADDR 0x31A2
...
static inline void __wpadResetModule(BOOL saveSimpleDevs)
{
	BTA_DmSendHciReset();
...
BOOL WPADSaveConfig(SCFlushCallback cb)
{
	...
	success &= SCSetWpadSpeakerVolume_(speakerVol);
	success &= SCSetWpadMotorMode_(rumble);

	if (success)
		SCFlushAsync((SCFlushCallback)cb);
	else if (cb)
		(*cb)(SC_STATUS_FATAL);
...
inline BOOL WPADiSendDPDCSB(struct WPADCmdQueue *cmdQueue, BOOL enabled,
                     WPADCallback *cb)
{
	BOOL success;
